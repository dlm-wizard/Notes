...
扰动计算：假设插入 &lt;“hollischuang”, V1> 键值对，经计算后得到 hash 值为 1011000110101110011111010011011。若当前 HashMap 为默认大小（16），在不进行扰动计算的情况下，最终得到 index 的结果为 11。由于 16 的 32 位二进制位为 00000000000000000000000000001111，所以，一个数字在和他进行按位与操作的时候，前 28 位无论是什么，计算结果都一样（因为0和任何数做与，结果都为0）。
...
final int hash(Object k) {
    ...
    h ^= k.hashCode();
    // 对key的hashCode进行扰动计算，把高位的特征和低位的特征组合起来，降低哈希冲突的概率
...
LinkedHashMap 就是在 HashMap 的基础上加了一条双向链表用来维护 LinkedHashMap 中元素的插入顺序或者 LRU 顺序。

<div align="center"> <img src="https://github.com/TangBean/MarkdownNotes/raw/master/JavaSE/pic/LinkedHashMap%E7%BB%93%E6%9E%84.png"/> </div><br>
