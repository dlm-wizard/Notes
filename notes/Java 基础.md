...
异常是发生在程序运行时阻碍程序正常执行的错误事件。

只要在 Java 语句执行中产生了异常，一个异常对象就会被创建，JRE就会试图寻找异常处理程序来处理异常。否则，将引发运行环境异常，JRE 终止程序执行。

**异常的层级：**  

Java把异常当作对象来处理，Throwable 作为所有异常类的超类，分为两种：  **Error**   和 **Exception**。其中 Error 用来表示 JVM 无法处理的错误，Exception 分为两种：

-   **受检异常**  ：在程序中能预期，并要尝试修复。需要用 try...catch... 语句捕获并进行处理；
-   **非受检异常**  ：源于糟糕的编程，例如除 0 会引发 Arithmetic Exception，此时程序崩溃并且无法恢复。

<div align="center"> <img src="https://cs-notes-1256109796.cos.ap-guangzhou.myqcloud.com/PPjwP.png" width="600"/> </div><br>

**异常处理的关键字：**  

-   **try-catch**  ：所描述的即是监控区域，即一段可能产生异常的代码和其异常处理代码；

-   **throw**  ：显式地创建并抛出异常对象来终止程序的正常执行；

-   **throws**  ：告知调用程序此方法可能会抛出的异常；

-   **finally**  ：不管是否有异常被捕获，finally 子句中的代码都被执行（回收资源）。警告！当其包含 return 语句时，会覆盖 try 语句块的返回值；
